...
spe <- make_spe_from_expr_data(expression_file,
                                hierarchy_file,
...
# TODO: allow metadata col to be specified directly
# currently picks the first column that is not "image"
meta_col <- metadata_cols[!tolower(metadata_cols) %in% "image"][1]
if (length(meta_col) > 0) {
    plot_cells_spatially(
        spe,
        parent_types = parent_types,
        colour_by = meta_col
